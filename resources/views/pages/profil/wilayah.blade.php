...
@section('content')
    <div class="space-y-8">
        {{-- Peta Interaktif --}}
        <section aria-labelledby="map-title" class="bg-white rounded-lg border border-green-100 shadow-sm overflow-hidden">
            <div class="bg-[#2E7D32] px-6 py-4 flex items-center justify-between">
                <h2 class="text-sm font-black text-white uppercase tracking-widest" id="map-title">Peta Administrasi Desa</h2>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div class="bg-white rounded-lg border border-green-100 shadow-sm p-5 flex flex-col items-center text-center group hover:border-green-300 hover:shadow-md transition-all">
                <div class="size-10 rounded-lg bg-green-100 flex items-center justify-center mb-3 group-hover:bg-[#2E7D32] transition-colors">
                    <flux:icon :name="$stat['icon']" class="size-5 text-[#2E7D32] group-hover:text-white transition-colors" />
                </div>
        ...
        <div class="grid sm:grid-cols-2 gap-6">
            <section aria-labelledby="borders-title" class="bg-white rounded-lg border border-green-100 shadow-sm overflow-hidden">
                <div class="bg-[#1B5E20] px-6 py-4 border-b border-green-900 flex items-center justify-between">
                    <h3 class="text-xs font-black text-white uppercase tracking-widest" id="borders-title">Batas Wilayah</h3>
                    <flux:icon.arrows-right-left class="size-4 text-green-400" />
                </div>
                <div class="p-6">
                    <ul class="space-y-0 divide-y divide-green-50">
                        @foreach([
                        ...
                        <li class="flex justify-between items-center py-3 px-1">
                            <span class="inline-flex items-center gap-2">
                                <span class="size-2 rounded-full bg-[#2E7D32]"></span>
                                <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest">{{ $batas['arah'] }}</span>
                            </span>
            ...
            <section aria-labelledby="geo-title" class="bg-white rounded-lg border border-green-100 shadow-sm overflow-hidden">
                <div class="bg-[#2E7D32] px-6 py-4 border-b border-green-900 flex items-center justify-between">
                    <h3 class="text-xs font-black text-white uppercase tracking-widest" id="geo-title">Orbitasi &amp; Geografis</h3>
                ...
                <div class="p-6">
                    <ul class="space-y-0 divide-y divide-green-50">
                        ...
                        <li class="flex justify-between items-center py-3 px-1">
                            <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest">{{ $geo['label'] }}</span>
        ...
        <section aria-labelledby="dusun-title" class="bg-white rounded-lg border border-green-100 shadow-sm overflow-hidden">
            <div class="bg-[#2E7D32] px-6 py-4 border-b border-green-700 flex items-center justify-between">
                <h2 class="text-sm font-black text-white uppercase tracking-widest" id="dusun-title">Wilayah Administratif Dusun</h2>
                <flux:icon.rectangle-group class="size-5 text-white/70" />
            </div>
            <div class="p-6 overflow-x-auto">
                <table class="w-full text-left text-xs font-bold uppercase tracking-widest">
                    <caption class="sr-only">Daftar wilayah administratif dusun di Desa Tatung</caption>
                    <thead class="bg-green-50 border-y border-green-100 text-green-800">
                        <tr>
                    ...
                    <tbody class="divide-y divide-green-50 text-slate-700">
                        @foreach([
