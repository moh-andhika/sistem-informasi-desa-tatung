...
@section('content')
    <article class="bg-white rounded-lg shadow-sm overflow-hidden mb-10">
        {{-- Featured Image --}}
        @if($berita->gambar)
            <div class="w-full relative aspect-video overflow-hiddenlue-50">
        ...
        <div class="p-6 sm:p-8">
            ...
            <div class="prose prose-slate prose-sm sm:prose-base max-w-none prose-headings:font-black prose-headings:uppercase prose-headings:tracking-normal prose-a:text-prt-primary hover:prose-a:text-prt-secondary prose-img: prose-strong:text-slate-900 leading-relaxed text-slate-700">
                @if($berita->ringkasan)
                    <p class="text-base text-slate-800 font-bold mb-6 border-l-4 border-prt-primary pl-4 py-1 italic leading-relaxed bg-blue-50 rounded-r-lg">
                        "{{ $berita->ringkasan }}"
                    </p>
                @endif
            ...
            <div class="mt-8 pt-6 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex gap-2">
                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->url()) }}" target="_blank" rel="noopener" class="size-9 rounded-lg bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-blue-600 hover:text-white transition-all shadow-sm" aria-label="Bagikan ke Facebook">
                        ...
                        <a href="https://wa.me/?text={{ urlencode($berita->judul . ' ' . request()->url()) }}" target="_blank" rel="noopener" class="size-9 rounded-lg bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-blue-600 hover:text-white transition-all shadow-sm" aria-label="Bagikan ke WhatsApp">
                ...
                <a href="{{ route('publik.publikasi.berita.index') }}" class="inline-flex items-center gap-2 text-xs font-black text-prt-primary uppercase tracking-wide hover:gap-3 transition-all rounded-lg px-4 py-2 hover:bg-blue-50">
