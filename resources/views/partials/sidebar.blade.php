...
<div class="space-y-8 pb-12">
    ...
    <section class="overflow-hidden  bg-white " aria-labelledby="widget-login-title">
        <div class="flex items-center justify-betweenlue-900/10 bg-linear-to-r from-prt-secondary to-prt-primary px-6 py-4">
            <h3 class="text-sm font-black text-white uppercase tracking-widest" id="widget-login-title">Layanan Mandiri</h3>
        ...
        <div class="p-8">
            <form action="{{ route('login') }}" class="space-y-5">
                <div class="space-y-2">
                    ...
                    <input type="text" id="sidebar-nik" name="nik" placeholder="Masukkan 16 digit NIK" class="w-full  bg-white p-3.5 text-sm outline-none transition-all focus: focus:ring-2 focus:ring-prt-accent/40  required>
                </div>
                <div class="space-y-2">
                    <label for="sidebar-pin" class="text-[11px] font-black text-slate-500 uppercase tracking-widest">PIN / Password</label>
                    <input type="password" id="sidebar-pin" name="password" placeholder="********" class="w-full  bg-white p-3.5 text-sm outline-none transition-all focus: focus:ring-2 focus:ring-prt-accent/40  required>
                </div>
                <button type="submit" class="w-full  bg-prt-secondary py-4 text-xs font-black uppercase tracking-widest text-white transition-colors hover:bg-prt-navy-dark focus:ring-2 focus:ring-prt-accent focus:ring-offset-2">Masuk Sekarang</button>
            </form>
    ...
    <section class="overflow-hidden  bg-white " aria-labelledby="widget-announcement-title">
        <div class="flex items-center justify-betweenlue-900/10 bg-prt-navy-dark px-6 py-4">
            <h3 class="text-sm font-black text-white uppercase tracking-widest" id="widget-announcement-title">Pengumuman Terkini</h3>
        ...
        <div class="divide-y divide-blue-50" role="list">
            @forelse($pengumuman as $item)
                <div class="group cursor-pointer p-5 transition-all hover:bg-blue-50" role="listitem">
                    <div class="mb-1.5 flex items-center gap-2 text-[10px] font-black uppercase tracking-tighter text-prt-primary">
                        <flux:icon.clock class="size-3.5" aria-hidden="true" /> 
                        <time datetime="{{ $item->published_at ?? $item->created_at }}">
                            {{ $item->published_at ? $item->published_at->format('d M Y') : $item->created_at->format('d M Y') }}
                        </time>
                    </div>
                    <h4 class="text-sm font-bold uppercase leading-normal tracking-tight text-slate-800 group-hover:text-prt-secondary">{{ $item->judul }}</h4>
                </div>
            @empty
                <div class="p-8 text-center text-slate-400 text-sm italic">Belum ada pengumuman saat ini.</div>
            @endforelse
        </div>
        <a href="#" class="block  bg-blue-50 py-3.5 text-center text-[10px] font-black uppercase tracking-widest text-prt-primary transition-colors hover:bg-blue-100 hover:text-prt-secondary">Arsip Pengumuman &rarr;</a>
    </section>

    {{-- Statistik Penduduk Widget --}}
    <section class="overflow-hidden  bg-white " aria-labelledby="widget-stats-title">
        <div class="flex items-center justify-betweenlue-900/10 bg-linear-to-r from-prt-secondary to-prt-primary px-6 py-4">
            <h3 class="text-sm font-black text-white uppercase tracking-widest" id="widget-stats-title">Statistik Penduduk</h3>
            <flux:icon.users class="size-5 text-prt-accent" aria-hidden="true" />
        </div>
        <div class="p-8 space-y-6">
            @foreach([
                ['label' => 'Total Penduduk', 'val' => $profil['penduduk'], 'color' => 'bg-prt-secondary', 'p' => 100],
                ['label' => 'Laki-Laki', 'val' => $profil['lk'], 'color' => 'bg-prt-primary', 'p' => 49],
                ['label' => 'Perempuan', 'val' => $profil['pr'], 'color' => 'bg-prt-accent', 'p' => 51],
                ['label' => 'Kepala Keluarga', 'val' => $profil['kk'], 'color' => 'bg-prt-navy-dark', 'p' => 100],
            ] as $stat)
            <div class="space-y-2">
        ...
        <a href="{{ route('infografis') }}" class="block  bg-blue-50 py-3.5 text-center text-[10px] font-black uppercase tracking-widest text-prt-primary transition-colors hover:bg-blue-100 hover:text-prt-secondary">Detail Infografis &rarr;</a>
    </section>
    ...
    <section class="overflow-hidden  bg-white " aria-labelledby="widget-location-title">
        <div class="flex items-center justify-betweenlue-100 bg-blue-50 px-6 py-4">
            <h3 class="text-sm font-black uppercase tracking-widest text-prt-navy-dark" id="widget-location-title">Wilayah Desa</h3>
            <flux:icon.map-pin class="size-5 text-prt-primary" aria-hidden="true" />
        </div>
