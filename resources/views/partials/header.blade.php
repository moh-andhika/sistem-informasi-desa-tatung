...
<header x-data="{ mobileMenuOpen: false }" class="sticky top-0 z-50 w-full transition-all duration-300" x-bind:class="{
    ...
    <div
        class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 transition-all duration-300 sm:px-6 lg:px-8">
        {{-- Logo & Brand --}}
        <div class="flex items-center gap-3">
            <a href="{{ route('home') }}" wire:navigate
                ...
                <img src="{{ asset('assets/ponorogo__sid__60A13U2.png') }}" alt="Logo Kabupaten Ponorogo"
                ...
                <div class="flex flex-col leading-tight">
                    ...
                    <span class="text-xs font-bold uppercase tracking-[0.18em] transition-colors duration-300"
        ...
        <nav class="hidden items-center gap-2 lg:flex" aria-label="Navigasi utama desktop">
            @foreach($navigationService->getPublicMenu() as $item)
                ...
                @else
                    <div class="relative" x-data="{ open: false }" x-id="['desktop-submenu']" x-on:mouseenter="open = true"
                        x-on:mouseleave="open = false" x-on:click.outside="open = false" x-on:focusin="open = true"
                        x-on:focusout="if (!$el.contains($event.relatedTarget)) open = false"
                        x-on:keydown.escape.stop="open = false">
                        <button type="button" x-on:click="open = !open" :aria-controls="$id('desktop-submenu')"
                            ...
                            <flux:icon.chevron-down class="size-3.5 transition-transform duration-200"
                        ...
                        <div x-show="open" x-transition:enter="transition ease-out duration-100"
                            ...
                            x-transition:leave-end="opacity-0 translate-y-1" :id="$id('desktop-submenu')"
                            class="absolute left-0 top-full z-50 w-60 pt-2" x-cloak>
                            <div class="flex flex-col  bg-white p-1.5 shadow-xl shadow-blue-100/50 ring-1 ring-black/5">
                                @foreach($item['subitems'] as $sub)
                                    ...
                                    @else
                                        <a href="{{ route($sub['route']) }}" wire:navigate x-on:click="open = false"
        ...
        <div class="flex items-center gap-3">
            ...
            {{-- Mobile Menu Button --}}
            <button type="button" x-on:click="mobileMenuOpen = !mobileMenuOpen" :aria-expanded="mobileMenuOpen"
                :aria-label="mobileMenuOpen ? 'Tutup menu navigasi' : 'Buka menu navigasi'" aria-controls="mobile-menu"
                aria-haspopup="true"
    ...
    <div id="mobile-menu" x-show="mobileMenuOpen" x-transition:enter="transition ease-out duration-200"
        ...
        class="absolute inset-x-0 top-full  bg-white shadow-xl lg:hidden" x-cloak>
        <div class="mx-auto max-h-[calc(100vh-4.5rem)] max-w-7xl space-y-1 overflow-y-auto px-4 py-4 sm:px-6">
            ...
            @auth
                <div class="mb-2 flex items-center gap-3  bg-blue-50 px-3 py-4">
                    ...
                    <div class="flex min-w-0 flex-col">
                        ...
                        <span
                            class="truncate text-xs font-semibold uppercase tracking-wide text-prt-primary">{{ $roleLabel }}</span>
            ...
            @foreach($navigationService->getPublicMenu() as $item)
                ...
                @if(empty($item['subitems']))
                    <a href="{{ route($item['route']) }}" wire:navigate x-on:click="mobileMenuOpen = false"
                ...
                @else
                    <div x-data="{ open: false }" x-id="['mobile-submenu']" class="space-y-1">
                        <button type="button" x-on:click="open = !open" :aria-controls="$id('mobile-submenu')"
                            :aria-expanded="open" aria-haspopup="true"
                            ...
                            <span>{{ $item['label'] }}</span>
                            <flux:icon.chevron-down class="size-5 text-slate-500 transition-transform duration-200"
                                x-bind:class="open ? '-rotate-180' : ''" />
                        </button>

                        <div x-show="open" x-transition:enter="transition ease-out duration-150"
                            x-transition:enter-start="opacity-0 -translate-y-1"
                            x-transition:enter-end="opacity-100 translate-y-0" :id="$id('mobile-submenu')"
                            class="space-y-1 pb-2 pl-4 pr-3" x-cloak>
                            @foreach($item['subitems'] as $sub)
                                @if(isset($sub['is_divider']) && $sub['is_divider'])
                                    <div class="my-2   aria-hidden=" true"></div>
                                @else
                                    <a href="{{ route($sub['route']) }}" wire:navigate x-on:click="mobileMenuOpen = false"
            ...
            <div class="mt-4 flex flex-col gap-2  pb-2 pt-4">
                @auth
                    <a href="{{ $dashboardRoute }}" wire:navigate x-on:click="mobileMenuOpen = false"
                ...
                @else
                    <a href="{{ route('login') }}" wire:navigate x-on:click="mobileMenuOpen = false"
                    ...
                    <a href="{{ route('register') }}" wire:navigate x-on:click="mobileMenuOpen = false"
